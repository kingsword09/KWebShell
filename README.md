...
Phase 10 is planned as two separate evidence clients: an HTML5 Capability Lab and a digest-pinned, LobeHub-class application benchmark. Both will use only the public facade, emit raw machine-readable reports, and fail explicitly when required browser evidence or workload artifacts are unavailable. See [the example and benchmark plan](docs/example-benchmark-plan.md); no example implementation is claimed yet.
