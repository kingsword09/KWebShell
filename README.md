...
Objective 10.2 adds the [application-scale workload benchmark](kweb-example-app-benchmark/README.md). Its digest-pinned synthetic workload exercises application routing, streamed Markdown and code, a 5,000-row virtual list, Worker/WebSocket/IndexedDB activity, image/font/audio decode, DevTools/CDP, native-frame sampling, process-tree resource use, Profile continuity, and shutdown. One warmup and ten measured cold/warm pairs retain 22 raw JSON samples, median/p95/worst summaries, and verified screenshots. Baselines bind the exact runtime and workload digests to a platform, architecture, and machine class; missing evidence and unmatched baselines are terminal. macOS arm64 passes the complete local run, while the same task and artifacts remain mandatory hosted Windows x64 and Linux x64 gates.
