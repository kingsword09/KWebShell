...
Objective 10.1 adds the runnable [HTML5 Capability Lab](kweb-example-html5-lab/README.md). It drives the public Engine/Profile/Page facade in isolated cold and warm JDK 25 processes against one persistent Profile and local origin; validates 37 locked browser and host-policy probes; obtains the exact Chromium/CDP identity and accessibility tree through an explicitly configured loopback endpoint; routes a trusted input event; and emits strict JSON, rendered HTML, and real `ComposeWindow` screenshots. Required failures, optional execution errors, empty evidence, non-contiguous public events, and missing persistence fail instead of degrading.

Objective 10.2 adds the [application-scale workload benchmark](kweb-example-app-benchmark/README.md). Its digest-pinned synthetic workload exercises application routing, streamed Markdown and code, a 5,000-row virtual list, Worker/WebSocket/IndexedDB activity, image/font/audio decode, DevTools/CDP, native-frame sampling, process-tree resource use, Profile continuity, and shutdown. One warmup and ten measured cold/warm pairs retain 22 raw JSON samples, median/p95/worst summaries, and verified screenshots. Baselines bind the exact runtime and workload digests to a platform, architecture, and machine class; missing evidence and unmatched baselines are terminal. macOS arm64 passes the complete local run. The PR matrix is temporarily Windows x64-only while the Windows process and benchmark gate is repaired; Linux and macOS complete verification remain explicit follow-up gates, and hosted screen capture is never treated as equivalent to local native-child evidence.
