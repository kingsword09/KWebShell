...
Objective 10.1 adds the runnable [HTML5 Capability Lab](kweb-example-html5-lab/README.md). It drives the public Engine/Profile/Page facade in isolated cold and warm JDK 25 processes against one persistent Profile and local origin; validates 37 locked browser and host-policy probes; obtains the exact Chromium/CDP identity and accessibility tree through an explicitly configured loopback endpoint; routes a trusted input event; and emits strict JSON, rendered HTML, and real `ComposeWindow` screenshots. Required failures, optional execution errors, empty evidence, non-contiguous public events, and missing persistence fail instead of degrading. macOS arm64 passes locally; Windows x64 and Linux x64 remain hosted PR gates before cross-platform publication. Objective 10.2, the digest-pinned LobeHub-class workload benchmark, remains the next implementation objective.
