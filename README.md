...
RFC 0030 adds the application packaging boundary above that signed release. A
closed target-complete manifest binds application identity, protocol/file
associations, helper ownership, capability resources, SBOM data, and immutable
`isPackaged` state. macOS/Windows use deterministic ZIP-compatible package
containers and Linux emits a deterministic Debian package; all package records
are covered by an explicit Ed25519 package statement. See the
[application package format](docs/application-package-format.md) and
[RFC 0030](docs/rfcs/0030-packaging-identity-associations.md).
