...
- Windows consumes the public `ComposeWindow.windowHandle` as the top-level
  ...
  custom destruction path, disables and hides the child, synchronously confirms
  focus has left its `Chrome_WidgetWin` subtree, drains eight CEF UI queue turns,
  and then destroys the Chromium child. Returning `false` is forbidden because
...
On Windows, `DoClose` returns `true`, blocks new child input, confirms focus loss,
and drains eight CEF UI queue turns before destroying the child. Only after
