...
| --- | --- | --- | --- | --- | --- | --- |
| A1 / common contract | The lifecycle API exposes typed identity, state, activation, quit, relaunch, participant, and error contracts with fixed limits. | Valid Unicode activation; invalid identity; zero limits; duplicate start; no raw argument field. | Common contract tests; schema/ABI review; all advertised targets. | `KWebApplicationLifecycle*`, `KWebApplicationActivationCodec`, and `KWebApplicationLifecycleContractTest`. | The three RFC 0006 records in `docs/rfcs/evidence/manifest.json`; lifecycle reports under `docs/rfcs/evidence/artifacts/0006/<sourceRevision>/<target>/`. | `PASS`: typed limits, canonical data, stable errors, state transitions, and duplicate-start/invalid-input cases are covered. |
| A2 / single instance | Exactly one real process owns the identity lease; a secondary forwards one authenticated activation and exits without CEF startup. | Two-process race; stale lease; crash recovery; wrong identity; stale nonce; simultaneous startup. | Two real packaged processes and native provider transcript on Windows, macOS, Linux. | `ApplicationLifecycleIntegrationMain`, `JvmKWebApplicationLifecycleBackend`, and the versioned native ABI providers. | Each target lifecycle report has `secondaryForwarded: true`, `unicodeFileForwarded: true`, `status: PASS`, and zero live native owners. | `PASS`: hosted two-process activation and authenticated forwarding passed on all three targets. |
| A3 / activation ordering | Canonical URI/file activations are authenticated, bounded, ordered, replay-bounded, and never silently dropped. | Unicode URI/file names; malformed URI; unregistered scheme; 64/65 event boundary; oversized frame; simultaneous quit/activation. | Common property tests plus native transport tests on all three targets. | `KWebApplicationActivationCodec`, controller queue/replay logic, and native frame validation. | Per-target activation reports retain ordered `activationEvents` and sequence values; the manifest retains their SHA-256 values. | `PASS`: canonicalization, UTF-8/frame limits, sequence ordering, replay capacity, and negative cases passed. |
| A4 / OS registration | Deep links and file opens arrive through RFC 0030 OS registration and retain observed identity/cleanup evidence. | Install, activate URI, activate Unicode file, multiple files, wrong package identity, remove and verify cleanup. | Real packaged registration/activation harness on Windows, macOS, Linux. | `JvmKWebApplicationLifecycleBackend.installAssociations/removeAssociations` and RFC 0030 package identity checks. | Every lifecycle report has non-empty install/remove digests and `registrationCleaned: true`; the three package reports are retained under RFC 0030. | `PASS`: registration identity, activation, Unicode file forwarding, and cleanup were observed on all targets. |
| A5 / shutdown | Normal quit closes Page/Profile/Engine/services before the lease and records graceful completion; forced termination is distinct. | Normal close; participant veto; participant failure; timeout; native crash/OS kill; close called twice. | Desktop integration with real CEF on all three hosted targets; shutdown timing transcript. | `KWebDesktopEngine.applicationShutdownParticipant`, `KWebApplicationLifecycleController.requestQuit`, and native engine integration. | Per-target `application-shutdown.json` records `outcome: GRACEFUL`, engine/native-service/profile stages, and zero live CEF owners; lifecycle reports also record graceful shutdown. | `PASS`: normal close and participant negotiation passed; forced/native failure paths remain typed non-graceful outcomes. |
| A6 / relaunch | Host-only relaunch starts exactly the verified RFC 0030 executable without exposing or accepting arbitrary arguments. | Packaged relaunch; unpackaged failure; wrong executable; concurrent relaunch; preserve/discard activation choice. | Process harness and package identity verification on all three targets. | `JvmKWebApplicationLifecycleBackend.requestRelaunch`, verified executable/package-root checks, and RFC 0030 package verifier. | Hosted package reports and lifecycle contract evidence bind relaunch to the verified package identity; no executable/argument field is accepted by the public API. | `PASS`: packaged-only host relaunch and invalid/unpackaged rejection are covered. |
| A7 / security and policy | Renderer cannot acquire lease, read raw process data, request quit/relaunch, or bypass authentication. | Generated renderer route inspection; forged sender; cross-user/remote sender; malformed payload; arbitrary executable request. | Common negative tests, native sender-auth tests, generated bridge inspection. | Host-only lifecycle descriptor, Windows PID/token authentication, Linux D-Bus credential checks, and migration negative fixtures. | Hosted lifecycle/provider artifacts contain only bounded activation/status data; no raw argv, secret, native pointer, or renderer control route is retained. | `PASS`: forged/remote sender and renderer-controlled lifecycle operations are rejected with typed failures. |
| A8 / platform feasibility | Windows, macOS, and Linux use their declared native APIs and fail explicitly when the session/facility is unavailable. | No D-Bus session; AppKit callback on wrong thread; Windows pipe remote sender; unsupported package mode. | Native provider probes and real hosted runtime artifacts on all three targets. | `application_lifecycle_abi.h/.cc`, `FfmApplicationLifecycle`, and the declared Windows named mutex/pipe, macOS AppKit/Launch Services, and Linux D-Bus providers. | Manifest records the provider identity and ABI-backed lifecycle report for `windows-x64`, `macos-arm64`, and `linux-x64`; strict native ABI tests passed. | `PASS`: all advertised providers passed their hosted target; unavailable facilities and invalid identities fail explicitly without fallback. |
| A9 / migration | Closed Electron lifecycle declarations map to typed Kotlin ownership; unsupported dynamic behavior blocks generation. | Valid mapping; dynamic listener; raw argv; arbitrary relaunch path; renderer quit. | JVM migration fixture tests and generated report. | `KWebElectronApplicationLifecycleMapper` and `KWebElectronApplicationLifecycleContractTest`. | Per-target `compatibility.json` artifacts and the migration contract binding are retained; the manifest pins Electron fixture major 44. | `PASS`: closed declarations map to typed ownership and dynamic/raw/arbitrary behavior is blocking. |
| A10 / documentation and evidence | Contract, package registration, user-facing lifecycle guidance, capability metadata, tests, and evidence are updated together. | Missing docs, stale association digest, skipped target, stale provider/ABI revision. | Governance check, documentation check, complete PR diff review. | This RFC, `docs/application-lifecycle.md`, `contracts.json`, CI aggregation, and the lifecycle/native/CEF tests. | `manifest.json` contains three current RFC 0006 records and retained artifacts; the evidence records include contract and runtime digests. | `PASS`: implementation, docs, capability/evidence bindings, and hosted artifacts are updated as one acceptance set. |
| A11 / universal completion | All applicable requirements are PASS on every advertised target and final review records the exact merged revision. | Any missing artifact, stale digest, failed target, skipped native verification, or changed contract after review blocks merge. | `:kweb-rfc-governance:check`, `git diff --check`, hosted aggregation, final row-by-row review. | Full PR implementation, hosted aggregation job, and this final acceptance review. | The three manifest records are the source of truth for the exact hosted source revision, run/attempt, contract digest, and artifact SHA-256 values. | `PASS`: A1–A10 are PASS, all three targets passed, and the reviewed manifest binds the exact hosted revision. |
...
- Review pass: Codex final acceptance review, same contributor as implementation;
  this is a separate review pass and not an independent-person approval.
- Scope: A1–A11 were reviewed against the complete implementation diff, the
  three hosted target results, the retained lifecycle and CEF shutdown reports,
  and the current rows in `docs/rfcs/evidence/manifest.json`.
- Revision/provenance: the exact hosted source revision, workflow ref, run,
  attempt, contract digest, and retained artifact hashes are recorded by the
  three RFC 0006 records in `manifest.json`; no revision is inferred from a
  local short SHA.
- Decision: `PASS` for A1–A11; RFC 0006 is `Implemented`.
