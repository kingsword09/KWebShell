...
|---|---|---|---|---|---|---|
| A1 / typed page envelope | Events expose page/profile/frame/origin identity, typed reason, bounded title/favicon data, and contiguous sequence. | Main/subframe events; embedded NUL replacement and oversized title; eight/9 canonical favicons; sequence starts at 1 and has no gaps. | Common contract tests, generated/native ABI tests, real hosted page fixture on macOS, Windows, Linux/X11. | `kweb-core/.../KWebPageContract.kt`; `kweb-desktop/.../KWebDesktopEngine.kt`; C ABI `kweb_browser_event`; `KWebDesktopContractTest`; `NativeEngineIntegrationMain`. | `docs/rfcs/evidence/artifacts/0008/336748c238e0e9a554d7c8dbda3db35ab79daf2e/{linux-x64,macos-arm64,windows-x64}/page-lifecycle/page-lifecycle-evidence.json`; hosted run `36083989978`, attempt 2. | **PASS** — all three real CEF transcripts contain the typed identity envelope and contiguous public sequence; ABI, Kotlin, and governance contract tests passed. |
| A2 / navigation order | Redirect, HTTP failure, history, reload, same-document and cross-origin navigation produce the declared ordered phases. | Redirect chain; 404; abort superseded navigation; hash/history; subframe navigation excluded from main-frame phases. | Real HTTP/HTTPS fixture and CEF transcript on all hosted targets. | `kweb-cef-native/src/browser_session.cc`; `KWebDesktopPage`; page-lifecycle redirect/hash/reload fixture. | Same three `page-lifecycle-evidence.json` artifacts at source revision `336748c`; hosted run `36083989978`, attempt 2. | **PASS** — redirect, same-document, reload, and terminal load assertions are retained for Linux, macOS, and Windows. |
| A3 / title and favicon | Title and favicon changes are bounded, origin-associated, and cleared at a new committed navigation. | Empty title; oversized title; multiple favicon URLs; cross-origin clear; subframe title ignored. | Real CEF display callbacks and event transcript on all hosted targets. | CEF display callbacks in `browser_session.cc`; public title/favicon mapping; page lifecycle fixture; HTML5 evidence mapping regression test. | Same three page-lifecycle artifacts; hosted renderer/page artifacts are retained under source revision `336748c`. | **PASS** — title/favicon bounds, canonical association, and navigation clearing are asserted in the retained transcripts. |
| A4 / before-unload | Before-unload requests are single-resolution, page-bound, five-second bounded, and default-cancel on timeout/close. | Proceed/cancel; duplicate/stale response; timeout; page/Profile close while pending; no renderer resolution. | Real JS before-unload fixture and native JS-dialog callback on all hosted targets. | `CefJSDialogHandler`; pending decision registry; `respondToBeforeUnload`; page lifecycle cancel/proceed/timeout/stale fixture. | Three page-lifecycle artifacts at `336748c`. | **PASS** — cancel, proceed, timeout-cancel, stale response, and close precedence are recorded on all advertised targets. |
| A5 / popup policy | CEF popup is canceled immediately; the host request defaults to denial and may create a separate Alloy child only in an explicit Compose owner. | Allow/deny/timeout; invalid owner; invalid bounds; cross-origin target; canceled JS popup proxy; no hidden top-level owner. | Real `window.open`/link fixture, event transcript, and native child hierarchy evidence on all hosted targets. | `OnBeforePopup`; popup decision registry; `respondToPopup`; RFC 0007 owner hierarchy; popup fixture. | Three page-lifecycle artifacts at `336748c`; RFC 0007 window-controls artifacts at the same revision. | **PASS** — deny, allow, timeout, invalid owner/bounds, owner binding, and no-`window.opener` rewrite are asserted on all targets. |
| A6 / renderer responsiveness | Unresponsive/responsive/terminated transitions are real, typed, single-terminal, and ordered before page close. | Busy-loop hang/recovery; crash; external kill; Profile/Engine close during unresponsive; no callbacks after close. | Real CEF renderer hang/crash fixtures and retained process/lifecycle transcript on all hosted targets. | `OnRenderProcessUnresponsive/Responsive/Terminated`; `NativeBrowser`; renderer crash ABI hook; stream cancellation fixture. | `docs/rfcs/evidence/artifacts/0008/336748c238e0e9a554d7c8dbda3db35ab79daf2e/{linux-x64,macos-arm64,windows-x64}/renderer-lifecycle/renderer-lifecycle-evidence.json`. | **PASS** — every target retains renderer termination reason/status, stream cancellation, terminal ordering, idempotent close, and zero native owners. |
| A7 / reload and terminal ownership | Controlled reload returns typed outcome, honors before-unload, and terminal page ownership is closed exactly once. | Normal/ignore-cache reload; pending popup/before-unload; renderer terminated; page/Profile/Engine close; duplicate close. | Kotlin lifecycle tests and real CEF reload/close fixture on all hosted targets. | `KWebPage.reload`; native reload ABI; `NativeBrowser` lifecycle state machine; page and renderer fixtures. | Page and renderer lifecycle artifacts at `336748c` for Linux, macOS, and Windows. | **PASS** — typed reload outcomes and exactly-once terminal ownership are evidenced across all three targets. |
| A8 / backpressure and concurrency | Slow subscribers cannot block CEF UI; popup/before-unload responses remain bounded and ordered. | Slow collector; event capacity boundary; concurrent response; close during callback; native callback after owner release. | RFC 0004 stream tests plus hosted callback stress on all targets. | `KWebPageEventStream`; callback dispatcher; bounded decision registries; RFC 0004 stream conformance tests. | `docs/rfcs/evidence/artifacts/0004/336748c238e0e9a554d7c8dbda3db35ab79daf2e/{linux-x64,macos-arm64,windows-x64}/stream-conformance/stream-conformance.json`. | **PASS** — bounded stream and callback conformance is green on every hosted target and page decisions remain bounded. |
| A9 / security and origin policy | No renderer can resolve popup/before-unload, create owners, access another page, or retain a bridge across cross-origin navigation. | Child frame; forged request id; cross-origin navigation; unconfigured page; raw native handle attempt. | Generated bridge inspection and real CEF negative fixtures on all hosted targets. | Exact-origin bridge dispatcher; page/profile policy checks; cross-origin reset and child-frame negative fixtures; desktop contract tests. | Page-lifecycle artifacts at `336748c`; hosted migration/governance artifacts for the same run. | **PASS** — forged/stale requests, child-frame isolation, cross-origin bridge reset, and owner/handle policy failures are typed and covered. |
| A10 / migration and docs | `webContents` events are classified individually; unsupported/unknown events block migration; docs/capability metadata match implementation. | Known supported events; rewrite-required events; unknown event name; popup policy mapping; renderer failure mapping. | Migration unit/golden tests, README, capability matrix, complete PR review. | `KWebElectronCapabilityMatrix`; migration golden files; Electron migration tests; RFC migration table and README. | Hosted migration artifacts plus `docs/rfcs/evidence/manifest.json` records for RFC 0008 and matrix rows. | **PASS** — supported, adapter, rewrite, and unsupported surfaces are individually classified and the four RFC 0008 rows are evidence-backed. |
| A11 / universal completion | Every applicable row passes on every advertised target and final review binds the tested contract/evidence revision. | Missing target, stale digest, skipped native test, changed contract after evidence, or unsupported advertised platform blocks merge. | Governance check, git diff check, hosted macOS/Windows/Linux matrix, final row review. | Complete PR diff; `contracts.json`; `manifest.json`; retained artifacts; final acceptance review below. | Hosted run `36083989978` attempt 2: all three verify jobs and hosted evidence recording passed; tested revision `336748c238e0e9a554d7c8dbda3db35ab79daf2e`. | **PASS** — all advertised targets passed real CEF verification, every applicable matrix row is backed, and the retained manifest binds the exact reviewed contract revision. |
...
- Reviewed revision: hosted PR merge revision
  `336748c238e0e9a554d7c8dbda3db35ab79daf2e`, with the final contract and
  evidence updates retained in this PR.
- Review pass: Codex final acceptance review by the same contributor; it is
  not an independent-person approval.
- Date: 2026-09-25.
- Hosted verification: CI run `36083989978`, attempt 2; macOS arm64,
  Windows x64, and Linux x64 verify jobs passed, and the hosted evidence
  recorder passed. The pre-refresh checked-in-evidence job was stale against
  the prior manifest; the retained manifest and artifacts are the reviewed
  replacement.
- Disposition: A1-A11 are `PASS`. The implementation is promoted from
