...
public class BridgeGenerator public constructor() {
    ...
    private fun generateBrowserStreamJavascript(schema: BridgeSchema): String {
        ...
        return buildString {
            ...
            appendLine("      globalThis.__kwebLastStreamQueuePeak = Math.max(globalThis.__kwebLastStreamQueuePeak || 0, frames.length);")
            appendLine("      armIdle();")
            appendLine("      wake();")
            appendLine("    };")
    ...
    private fun typescriptStreamRuntime(schema: BridgeSchema): String {
        ...
        appendLine("  const frames: Array<{ sequence: number; payload?: unknown }> = [];")
        ...
        appendLine("    if (done) return;")
        appendLine("    let frame: { kind?: string; sequence?: number; payload?: { bytes?: string }; reason?: string };")
        appendLine("    try { frame = JSON.parse(response); }")
        appendLine("    catch { terminate(\"bridge.stream.frame-invalid\", \"The stream frame is not valid JSON.\"); return; }")
        appendLine("    if (frame.kind === \"terminal\") {")
        appendLine("      done = true;")
        ...
        appendLine("    }")
        appendLine("    frames.push({ sequence: frame.sequence, payload: frame.payload });")
        appendLine("    (globalThis as { __kwebLastStreamQueuePeak?: number }).__kwebLastStreamQueuePeak = Math.max((globalThis as { __kwebLastStreamQueuePeak?: number }).__kwebLastStreamQueuePeak || 0, frames.length);")
        appendLine("    armIdle();")
        appendLine("    wake();")
        appendLine("  };")
